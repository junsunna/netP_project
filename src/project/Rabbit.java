...
//        gamePanel.repaint(); // 패널 다시 그리기
    ...
    private boolean upCharacter() {
        ...
        List<Rectangle> platforms = mainMap.getPlatforms();
    	for (Rectangle platform : platforms) {
    		if (new Rectangle(position.x + 50, position.y + 25 - JUMPSPEED, 85, 93).intersects(platform)) {
...
//					position.x = position.x - 1;
	...
	public void gameLoop() {
	    if (!loop) {
	        loop = true;
	        Timer timer = new Timer(16, e -> { // 약 60fps로 실행
	            // 중력 효과 적용
	            if (!up && !down) {
	                if (!downCharacter()) { 
	                    down(); // 아래로 이동
	            ...
	            // Rabbit 충돌 확인
	            if (Math.abs(position.x - 1500) <= 50 && Math.abs(position.y - 250) <= 50) {
	                mainMap.changeTileState("Rabbit"); // Rabbit 캐릭터로 타일 전환 호출
	            }
	        });
	        timer.start();
	    }
