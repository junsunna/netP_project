...
public class ObjServerGUI {
	...
	public ObjServerGUI(int port) {
		this.port = port;
		frame = new JFrame("ObjServer GUI");
...
//			Object message; // 객체로 받기
			String message; // toString으로 문자열 파싱하여 받기
			while (true) {
				message = ((TestMsg)in.readObject()).toString();
//				message = in.readObject(); Object 타입으로 업캐스팅 / 어떤 객체든 받아냄
//				System.out.println("클라이언트 메시지: " + message); // 디버그
//				String msg = Integer.toString(message);      // 파싱
